...
const fetchPosts = async ({ hash, sig }: { hash?: string; sig?: string }) => {
	const url = new URL(
		`/api/devprotocol:clubs:plugin:posts/${props.feedId}/search/has:option/poll`,
...
onMounted(async () => {
	...
	const feeds = props.options.find((item) => item.key === 'feeds')?.value
	feed.value = feeds?.find((item) => item.id === props.feedId)
